...
const dialog = {
  ...
  init() {
    ...
    this.$url.addEventListener('input', () => autoHeight(this.$url));
  ...
  show(type, menuId) {
    ...
    if (type === 'add') {
      ...
      if (type === 'clone') this.$ele.dataset.id = menuItemsApi.NEW_ID;
    }
    this.$name.focus();
...
function handleMainClick(event) {
  ...
  if ($item && !['INPUT', 'BUTTON'].includes(tagName)) {
    ...
    $item = $('.item.active');
    if (!$item) {
      alert(L("cloneMenuTipForSelectOne"));
      return;
    }
  ...
  if (tagName === 'BUTTON') {
    ...
    switch(target.classList[0]) {
      case 'add':
        dialog.show('add', menuItemsApi.NEW_ID);
        break;
      case 'clone':
        menuId = $item.dataset.id;
        dialog.show('clone', menuId);
        break;
      case 'edit':
        menuId = $item.dataset.id;
        dialog.show('edit', menuId);
        break;
      case 'delete':
        menuId = $item.dataset.id;
        $item.remove();
