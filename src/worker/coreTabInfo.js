...
function titleTemplate(hostname, data, extraStatus) {
  let titleInfo;
  let { ip, type, ipClass, location, countryCode, ipFromDNS, title } = data;
...
${L('location')}:  ${location}${countryCode ? ' - ' + countryCode : ''}
`;
      break;
  }
...
function updateActionDetails(tabId, hostname, details) {
  ...
  let titlePromise = iconPromise.then(() => {
    title = title.trim();
    if (isFF) {
      title = title.replace(/^.*$/gm, (line) => ` ${line} `);
    } else {
      title +=  '\n';
    }
